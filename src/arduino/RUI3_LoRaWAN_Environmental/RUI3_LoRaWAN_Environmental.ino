...
#define TRY_SEND_REQUEST 10 // try calling api.lorawan.send()
...
#define OTAA_DEVEUI   {0xAC, 0x1F, 0x09, 0xFF, 0xFE, 0x0E, 0x3A, 0x30} // AC1F09FFFE0E3A30
#define OTAA_APPEUI   {0x0E, 0x0D, 0x0D, 0x01, 0x0E, 0x01, 0x02, 0x0E} // 0E0D0D010E01020E
#define OTAA_APPKEY   {0xb5, 0x12, 0xfb, 0x72, 0xfc, 0x87, 0x20, 0x2a, 0xb3, 0x90, 0xc1, 0xbe, 0x8e, 0x46, 0x99, 0xf6} // b512fb72fc87202ab390c1be8e4699f6

/** Packet buffer for sending */
uint8_t collected_data[64] = { 0 };

teapot::payload::v1::EnvironmentalSensorTypeC payload;
teapot::bwlr1e::Application app;
...
void uplink_routine()
{
    ...
    /** Payload of Uplink */
    uint8_t out[sizeof(teapot::payload::v1::EnvironmentalSensorTypeC::data)];
    float temperature, humidity, pressure, voltage;
    app.GetEnvironmentalData(temperature, humidity, pressure);
    voltage = app.ReadBatteryVoltage();
    payload.data.temperature = temperature * 100;
    payload.data.humidity = humidity * 100;
    payload.data.pressure = pressure;
    payload.data.voltage = voltage * 10000;
    
    Serial.printf("Temperature: %f, Humdity: %f, Pressure: %f, Voltage: %f\n", 
                  temperature,
                  humidity,
                  pressure,
                  voltage );

    Serial.printf("Temperature: %ld, Humdity: %ld, Pressure: %ld, Voltage: %ld\n", 
                  payload.data.temperature,
                  payload.data.humidity,
                  payload.data.pressure,
                  payload.data.voltage );
    payload.GetAsBytes( out, sizeof(out) );
  ...
  while( !api.lorawan.send(sizeof(out), (uint8_t *)out, 2, true, 1) && try_send_request--) {
